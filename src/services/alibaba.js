...
function extractRating(storeEvaluates) {
  ...
  const allReview = storeEvaluates.find(
    (e) => e.title === "All Product Review",
  );
...
async function searchAlibabaDataHub({
  keyword,
  pageSize = 20,
  quantity = 1000,
}) {
  ...
  try {
    ...
    const mapped = resultList.slice(0, pageSize).map((entry, index) => {
      ...
      const moq =
        parseInt(item.sku?.def?.quantityModule?.minOrder?.quantity) || 1;
      ...
      return {
        ...
        storeUrl: seller.storeUrl ? `https:${seller.storeUrl}` : "",
      };
    });

    // ── Filter out junk items (accessories, hardware) ─────────────────────────
    // ── Smart filter based on keyword type ──────────────────────────────────────
    const isMachinery =
      /machine|machinery|equipment|industrial|automatic/i.test(keyword);
    const minPrice = isMachinery ? 100 : 5; // machines are expensive
    const maxPrice = isMachinery ? 500000 : 100000;

    const filtered = mapped.filter((item) => {
      if (item.price === 0) return false; // no price
      if (item.price < minPrice) return false; // too cheap (likely accessory)
      if (item.price > maxPrice) return false; // unrealistic
      return true;
    });
